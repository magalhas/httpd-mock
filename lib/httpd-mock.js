...
HttpdMock.prototype.createWebServices = function (webServices) {
  ...
  var putOrPost = function (routes, req, res) {
    webServicePath = req.route.path.substr(this.options.servicesPrefix.length);
    var target = routes[webServicePath];
    // If namespace mode
    if (target.indexOf('.json') === -1) {
      var id = req.params.id;
      var data = req.body;
  ...
  if (webServices && webServices.post) {
    for (webServicePath in webServices.post) {
      ...
      this.getInstance().post(webServicePath, putOrPost.bind(this, webServices.post));
    }
  }
  if (webServices && webServices.put) {
    for (webServicePath in webServices.put) {
      this.prepareNamespace(webServices.put[webServicePath]);
      webServicePath = this.options.servicesPrefix + webServicePath;
       if (this.options.output) console.log('PUT ' + webServicePath);
      this.getInstance().put(webServicePath, putOrPost.bind(this, webServices.put));
    }
  }
  return this;
};
